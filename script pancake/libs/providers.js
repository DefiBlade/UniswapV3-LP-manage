...
const { Environment, CurrentConfig } = require("../config.js");
...
async function sendTransaction(transaction) {
  if (CurrentConfig.env === Environment.WALLET_EXTENSION) {
    await sendTransactionViaExtension(transaction);
  } else {
    await sendTransactionViaWallet(transaction);
  }
}
...
async function sendTransactionViaWallet(transaction) {
  // console.log("sendTransactionViaWallet");
  if (transaction.value) {
    transaction.value = BigNumber.from(transaction.value);
  }

  transaction.gasPrice = ethers.utils.parseUnits("6", "gwei");
